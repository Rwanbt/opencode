...
| W4 — llama-server flags | **Done (partial per spec)** | `--mmap`, `--slots`, `--slot-save-path`, `--cache-reuse 256`, and speculative decoding via `OPENCODE_DRAFT_MODEL` / sibling drafter detection added. `--prompt-cache` intentionally skipped (server-mode API difference — backlog). |
...
- **W4**
  ...
  3. `OPENCODE_DRAFT_MODEL=<abs>` → forces the given path; `OPENCODE_DRAFT_FORCE=1` bypasses the VRAM guard.
- **W6**
...
- **W4 / speculative decoding** — VRAM headroom heuristic is coarse: 4 GiB constant regardless of draft model size. A too-large drafter may still OOM llama-server on marginal hardware; mitigated by `OPENCODE_DRAFT_FORCE` opt-out pattern reversed (guard is the default). Watch stderr on first real user.
