...
1. **[ ] Décision explicite en attente** : `ViewerPanel` doit-il rester monté pendant la transition édition↔lecture (masquer/afficher au lieu de démonter/remonter) ? Gain potentiel : élimine tout le coût du remount Pierre (mesuré à 94.2 ms avant Phase 2, devenu le poste dominant restant maintenant que C1 est corrigé). Risque : changement structurel de `file-tabs.tsx` (`<Show>` → masquage), impact sur le cycle de vie CodeMirror à valider (ne doit pas rester monté en double, gestion du focus/scroll). **Décision reportée à l'utilisateur avant implémentation** — même règle que la Phase 2 (Architectural change discipline, `<Show>` dans `file-tabs.tsx` est un point d'architecture central déjà documenté comme tel dans ce plan).
2. **[x] Remount Solid conservé pour l'instant (en attendant la décision du point 1)** : le transfert de contenu est déjà propre — C1 (seed synchrone) + C2 (mémoïsation) garantissent que la nouvelle instance reçoit le contenu final correct immédiatement, sans repasser par un aller-retour réseau ni un recalcul redondant du checksum. Aucun changement supplémentaire nécessaire tant que le point 1 n'est pas tranché.
