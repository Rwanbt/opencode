...
describe("NativeWorkflowRuntimePort (directive 31)", () => {
  test("start surfaces the first step for dispatch; complete advances; approval step maps to human.approval", async () => {
    const ctx = freshPort(); try {
      ...
      const afterFirst = await ctx.port.complete(state.workflowId, { bytes: 12 })
      expect(afterFirst.outputs).toEqual([{ bytes: 12 }])
      const afterSecond = await ctx.port.complete(state.workflowId, { approved: true })
      expect(afterSecond.nextStep).toBe(2)
      const done = await ctx.port.complete(state.workflowId, { final: true })
  ...
  test("RESTART: resume rediscovers from durable facts; cancel persists and fences stale completion", async () => {
    ...
    try {
      ...
      const started = await first.start(def("wf-9"))
      await first.complete(started.workflowId, { step: 0 })
      ...
      const resumed = await second.resume(started.workflowId)
      expect(resumed.nextStep).toBe(1); expect(resumed.outputs).toEqual([{ step: 0 }])
      const cancelled = await second.cancel(started.workflowId)
  ...
  test("two runs of one definition stay isolated and pin versions across publication", async () => {
    const ctx = freshPort(); try {
      ...
      const first = await ctx.port.start(v1)
      const second = await ctx.port.start(v1)
      ...
      const third = await ctx.port.start(v2)
      expect(third.workflowId).not.toBe(first.workflowId)
      expect(third.versionId).not.toBe(first.versionId)
      expect((await ctx.port.resume(first.workflowId)).versionId).toBe(first.versionId)
      expect((await ctx.port.resume(second.workflowId)).versionId).toBe(second.versionId)
      expect((await ctx.port.resume(third.workflowId)).versionId).toBe(third.versionId)
    } finally { ctx.port.close(); rmSync(ctx.dir, { recursive: true, force: true, maxRetries: 5, retryDelay: 50 }) }
  })
})
