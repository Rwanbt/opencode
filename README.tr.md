...
- `--cache-type-k/v q4_0` — Hadamard rotasyonlu KV önbelleği (%72 bellek tasarrufu)
- `--fit on` — Bağlam boyutunu ve GPU katman yerleşimini mevcut VRAM'e göre otomatik ayarlar
- Spekülatif kod çözme (`--model-draft`) ile VRAM Koruması (< 1.5 GB boş olduğunda otomatik devre dışı)
...
- Önceden seçilmiş katalog: Gemma 4 E4B, Qwen 3.5 (4B/2B/0.8B), Phi-4 Mini, Llama 3.2
...
- **APK'da statik ikili dosyalar** — Bun, Git, Bash, Ripgrep (aarch64-linux-musl) ilk başlatmada çıkarılır (~15s)
...
- **KV önbellek kuantizasyonu** — Hadamard rotasyonlu `--cache-type-k/v q4_0` (%72 bellek tasarrufu)
