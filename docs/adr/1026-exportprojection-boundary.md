...
- Le cursor d'export (`runtime.ts`) est en mémoire par instance de processus, pas persisté en DB — un redémarrage repart de l'event le plus récent au moment du boot, sans rattraper l'historique antérieur. Documenté comme limitation Phase 4 (pas de backfill).
