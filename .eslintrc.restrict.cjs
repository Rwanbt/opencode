 ...
 *   - packages/ui/src/components/icon.tsx:129                 // JSX innerHTML={icons[name]} — static icon registry (dev-controlled SVG strings)
 *   - packages/ui/src/components/file-ssr.tsx:166             // JSX innerHTML={prerenderedHTML} — SSR-only (isServer guard), prerendered diff HTML
 *   - packages/web/src/components/share/content-bash.tsx:51-52  // Shiki-rendered HTML (SolidJS innerHTML={…})
 *   - packages/web/src/components/share/content-code.tsx:27   // Shiki codeToHtml output (escapes user input before HTML)
 *   - packages/web/src/components/share/content-markdown.tsx:53  // DOMPurify-sanitized marked output
 *   - packages/app/src/components/settings-general.tsx:915    // QR code SVG string generated by qrcode lib, not user HTML
 *
