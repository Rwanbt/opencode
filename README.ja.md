...
- `--cache-type-k/v q4_0` — Hadamard 回転 KV キャッシュ（72% メモリ節約）
- `--fit on` — 利用可能な VRAM に応じてコンテキストサイズと GPU レイヤー配置を自動調整
- 投機的デコーディング（`--model-draft`）と VRAM ガード（空き容量 < 1.5 GB で自動無効化）
...
- 事前キュレーション済みカタログ：Gemma 4 E4B, Qwen 3.5 (4B/2B/0.8B), Phi-4 Mini, Llama 3.2
- モデルサイズに応じた動的出力トークン
...
- **APK 内の静的バイナリ** — Bun、Git、Bash、Ripgrep (aarch64-linux-musl)、初回起動時に展開（~15秒）
...
- **KV キャッシュ量子化** — Hadamard 回転付き `--cache-type-k/v q4_0`（72% メモリ節約）
