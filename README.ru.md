...
- `--cache-type-k/v q4_0` — KV-кэш с поворотом Адамара (экономия памяти 72%)
- `--fit on` — автоматическая подстройка размера контекста и размещения GPU-слоёв под доступную VRAM
